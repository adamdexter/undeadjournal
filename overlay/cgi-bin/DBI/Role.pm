...
sub connection_bad {
    ...
    # Single-server instance: there is no replication to check, so the handle is
    # good as long as it exists. (The original code ran SHOW REPLICA STATUS, which
    # errors with 1227 on MariaDB on every check and left the handle in an error
    # state that made later queries fail in some request paths.)
    return 0;
}
